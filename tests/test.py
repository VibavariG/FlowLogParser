...
import os
from FlowLogParser import load_lookup_table, parse_flow_logs, map_logs_to_tags, write_output,load_id_protocols_to_hashmap
...
class TestFlowLogParser(unittest.TestCase):
        ...
        self.test_cases = [
            {
                ...
                "id_protocol_map_file": "data/id_protocol_map.txt",     #using the map file unser data/
    ...
    def run_test_case(self, test_case):
        with patch("FlowLogParser.FLOW_LOG_FILE", test_case["flow_log_file"]), \
             ...
             patch("FlowLogParser.ID_PROTOCOL_MAP", test_case["id_protocol_map_file"]), \
            ...
            id_protocol_map = load_id_protocols_to_hashmap(test_case["id_protocol_map_file"])

            # Running the main processing functions
            tag_counter, port_protocol_counter, untagged_count = map_logs_to_tags(
                logs, lookup_table, id_protocol_map
            )
            ...
            with open(test_case["actual_output_file"], 'r') as actual, \
                ...
                actual_content = actual.read().strip().lower()
                expected_content = expected.read().strip().lower()
